...
function _signerContrat(){{var b=document.getElementById('sign-badge');var a=document.getElementById('sign-actions');b.textContent='\u2714 Signe le '+new Date().toLocaleDateString('fr-FR');b.style.background='rgba(16,185,129,.2)';b.style.color='#10b981';a.innerHTML='<div style="padding:8px 12px;background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.2);border-radius:8px;font-size:.82rem;color:#10b981">\u2714 Contrat signe electroniquement le '+new Date().toLocaleDateString('fr-FR')+' a '+new Date().toLocaleTimeString('fr-FR',{{hour:'2-digit',minute:'2-digit'}})+' par {nom} {prenom}</div>';localStorage.setItem('contrat_signe_{nom}',new Date().toISOString());}}
